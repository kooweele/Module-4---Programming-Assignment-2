...
plot4 <- function() {
        ...
        ### subsetting sources of pollution from Coal Combusion based on Shortname
        df <- subset(df, grepl("Comb", Short.Name, ignore.case = TRUE))
        df <- subset(df, grepl("Coal", Short.Name, ignore.case = TRUE))
        sum_emissions_by_yr <- ddply (df, .(year), summarize, total.pm25 = sum(Emissions))
        plot(sum_emissions_by_yr$year , sum_emissions_by_yr$total.pm25, type = "l", xlab ="Year", ylab = "Total PM25/Tonnes", pch = 23, main="Emissions by Coal Combustion Sources (1999-2008)")
