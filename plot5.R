...
plot5 <- function() {
        ...
        plot_file_name="plot5.png"
        unlink(plot_file_name)
        pngfile = png(filename=plot_file_name)
        device_num <- dev.cur() 
        df <- extract_data() ## extract the raw data
        ### subsetting sources of pollution from Vehicles and then fips = Baltimore City
        ...
        plot(sum_emissions_by_yr$year , sum_emissions_by_yr$total.pm25, type = "l", xlab ="Year", ylab = "Total PM25/Tonnes", pch = 23, main="Emissions from Motor Vehicle Sources \n in Baltimore City (1999-2008)")
